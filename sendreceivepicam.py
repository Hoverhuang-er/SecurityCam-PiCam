...
## @package rpiPiCam
#  This script allow the control of the rpiPiCam expansion board for Raspberry Pi.
#
#  V1.0 allow only to send regular message on the PiCam Network.
#  syntax is :
#  sendPiCam MESSAGE 
#  where MESSAGE is an HEXA string encoded. Can be 2 to 24 characters representing 1 to 12 bytes.
#  Example : sendPiCam 00AA55BF to send the 4 bytes 0x00 0xAA 0x55 0xBF
# 
...
if WaitFor(ser, 'OK', 'ERROR', 3) :
	print('PiCam Modem OK')
else:
	print('PiCam Modem Init Error')
	ser.close()
	exit()

ser.write('ATE0\r')
if WaitFor(ser, 'OK', 'ERROR', 3) :
	print('PiCam Modem echo OFF')
else:
	print('PiCam Modem Configuration Error')
	ser.close()
	exit()
